...
description: Some tips on optimizing your prover setup.
...
<Steps>
  <Step title="Configure a Single GPU Instance">
    ```yml compose.yml
    gpu_agent0: &gpu
    image: agent
    runtime: nvidia
  ...
  <Step title="Verify that No Other GPU Definitions Are Present in the Compose File">
  </Step>
  <Step title="Confirm the `SEGMENT_SIZE` Is Set to the Maximum as Determined Above in the `.env.broker` File">
  </Step>
  <Step title="Execute the Test Harness">
    ```bash Terminal
    RUST_LOG=info bento_cli -c 4096
    ```
  </Step>
  <Step title="Confirm Single GPU Utilization Using `nvtop`">
    ![Nvtop diagram](/images/nvtop.png)
  </Step>
  <Step title="Review the Effective Hz">
    ```bash Terminal
